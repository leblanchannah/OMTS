        ...
        <div class="container">
            <div class="row">
                ...
                <?php
                    ...
                    $row= $dbh->query('select phone_number from supplier where name='.$supplier);
                    $results = $row->fetchAll(PDO::FETCH_ASSOC);
                    $suppliernum = $results[0]['phone_number'];


                    $stmt = $dbh->prepare("insert into movie values(:title, :director, :length, :plot_synopsis, :actors, :production_company, :supplier_name,:supplier_phone_number)");
                    ...
                    $stmt->bindParam(':supplier_name', $supplier);    
