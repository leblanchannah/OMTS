            ...
            <div class="row">
                ...
                <div class="jumbotron">
                                ...
                                <?php
                                    ...
                                    $sth = $dbh->prepare('SELECT title,fname,lname,seats_reserved,booking_time,cancel_flag FROM (SELECT fname,lname,seats_reserved,booking_time,cancel_flag,movie_id FROM reserves INNER JOIN customer ON customer.account_number=reserves.account_number WHERE reserves.account_number='.$_POST['customername'].') reservations INNER JOIN movie ON movie.movie_id = reservations.movie_id ORDER BY booking_time ASC');
                                    // add try catch
                                    $sth->execute();
                                    $rows = $sth->fetchAll();
                                    // account number is id for the row so we can delete the rows with jquery later
                                    if (count($rows) == 0) {
                                        echo 'This user has not purchased any tickets.';
                                    
                                    } else {
                                       echo ' <!-- show all of users past purchase history-->
